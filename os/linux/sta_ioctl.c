...
#ifdef RTMP_MAC_USB
		strncpy(name, "RT3070 Wireless", IFNAMSIZ);
#endif // RTMP_MAC_USB //
	return 0;
}
		...
		struct iw_point *wrq, PSTRING extra)
{
    ...
    switch(subcmd)
    {
        ...
        case SHOW_DRVIER_VERION:
            sprintf(extra, "Driver version-%s,build:%s, %s %s\n", STA_DRIVER_VERSION, DRIVER_BUILD_VERSION, __DATE__, __TIME__ );
            wrq->length = strlen(extra) + 1; // 1: size of '\0'
            break;
#ifdef DOT11_N_SUPPORT
        case SHOW_BA_INFO:
					...
					else
    {
        ...
        switch (alg) {
    		...
    		case IW_ENCODE_ALG_WEP:
				...
				if (pAdapter->StaCfg.GroupCipher == Ndis802_11WEPEnabled ||
				...
				{			
					...
					//PMAC_TABLE_ENTRY pEntry = &pAdapter->MacTab.Content[BSSID_WCID];
					RTMPAddWcidAttributeEntry(pAdapter, BSS0, keyIdx, pAdapter->SharedKey[BSS0][keyIdx].CipherAlg, pEntry);
    ...
    IN  INT                 cmd)
{
	...
	switch(cmd & 0x7FFF) {
        ...
        case OID_802_11_BSSID_LIST_SCAN:
			...
			if ((pAd->RalinkCounters.LastOneSecTotalTxCount > 30) || (pAd->RalinkCounters.LastOneSecRxOkDataCnt > 30))
            {
    ...
    IN  PSTRING          arg)
{
    ...
    if( strlen(arg) <= MAX_LEN_OF_SSID)
    {
        ...
        if (pAdapter->StaCfg.BssType == BSS_ADHOC)
            pAdapter->StaCfg.bNotFirstScan = FALSE;
...
#ifdef DOT11_N_SUPPORT
        //Added by Eddy
        pAdapter->StaCfg.CfgPhyMode = pAdapter->CommonCfg.PhyMode;
        switch(pAdapter->CommonCfg.PhyMode) {
            case PHY_11ABGN_MIXED:
                pAdapter->CommonCfg.PhyMode = PHY_11ABG_MIXED;
                break;
            case PHY_11N_2_4G:
                pAdapter->CommonCfg.PhyMode = PHY_11N_2_4G;
                break;
            case PHY_11GN_MIXED:
                pAdapter->CommonCfg.PhyMode = PHY_11G;
                break;
            case PHY_11AN_MIXED:
                pAdapter->CommonCfg.PhyMode = PHY_11A;                    
                break;
            case PHY_11BGN_MIXED:
                pAdapter->CommonCfg.PhyMode = PHY_11BG_MIXED;                    
                break;
            case PHY_11AGN_MIXED:
                pAdapter->CommonCfg.PhyMode = PHY_11ABG_MIXED;                    
                break;
            case PHY_11N_5G:
                pAdapter->CommonCfg.PhyMode = PHY_11N_5G;                    
                break;
        } //End of switch
        pAdapter->CommonCfg.DesiredPhyMode = pAdapter->CommonCfg.PhyMode;
        if (pAdapter->StaCfg.CfgPhyMode >= PHY_11ABGN_MIXED) {
            RTMPSetPhyMode(pAdapter, pAdapter->CommonCfg.PhyMode);
...
#ifdef DOT11_N_SUPPORT
        //Added by Eddy
        if ((pAdapter->CommonCfg.PhyMode != pAdapter->StaCfg.CfgPhyMode)
            && (pAdapter->StaCfg.CfgPhyMode >= PHY_11ABGN_MIXED)) {
            pAdapter->CommonCfg.PhyMode = pAdapter->StaCfg.CfgPhyMode;
            pAdapter->CommonCfg.DesiredPhyMode = pAdapter->CommonCfg.PhyMode;
            RTMPSetPhyMode(pAdapter, pAdapter->CommonCfg.PhyMode);
            SetCommonHT(pAdapter);
        }
